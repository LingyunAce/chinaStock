...
### 场景 47：金叉/死叉信号扫描（多角度技术分析）

```
用户："大唐电信有没有金叉信号？" / "XX 股票是不是死叉了？" / "多空力道如何？"

AI 步骤：
1. 拉取全指标：westock-data technical sh600198 --group all
2. **必须参考** references/technical-cross-signals.md 中的"金叉判定矩阵"
3. 按 8 大主流指标分别判断：
   - MACD（DIF vs DEA）→ 是否金叉/死叉
   - KDJ（K vs D）→ 是否金叉，J 值所处区间（超卖/中位/超买）
   - MA5/MA10、MA5/MA20 → 短期均线是否上穿
   - EMA12/EMA26 → 指数均线金叉状态
   - DMI（PDI vs MDI）→ 多空力道权威判断
   - TRIX vs TRIXMA → 长期趋势金叉
   - PSY vs PSYMA → 人气辅助判断
4. 结合超买/超卖区域（KDJ-J、RSI6/12、WR、CCI、BIAS）评估金叉有效性
5. 按"金叉强度评分"打分（0-10 分）
6. 输出标准化扫描矩阵 + 操作建议
7. 识别四大典型场景（强势金叉/超卖反弹/死叉预警/假金叉陷阱）

输出格式必须包含：
- 主流金叉指标扫描矩阵（Markdown 表格，含🔴🟢🟡状态标识）
- 已金叉/接近金叉/死叉指标数量统计
- 综合评级（金叉强度 X/10 + 多空力道）
- 明确的"是否可买"答复 + 关键触发条件 + 止损位/目标位
```
...
AI 步骤：
1. westock-data technical sh600000 --group all
2. 重点关注 DMI 三要素：
   - PDI（多头力量指标）
   - MDI（空头力量指标）
   - ADX（趋势强度，ADX 上升 = 趋势确立）
3. 辅助看 MACD 柱状（红柱扩大 = 多头动能增强）
4. 结合 SAR（停损转向）判断趋势方向
5. 综合判断当前是单边市还是震荡市
   - 若 ADX > 40 且 PDI > MDI：强多头单边
   - 若 ADX > 40 且 MDI > PDI：强空头单边
   - 若 ADX < 20：震荡市，趋势指标失效
6. 输出多空力道评级与对应操作建议
```
...
**金叉/死叉/多空力道分析必备参考**：[technical-cross-signals.md](./technical-cross-signals.md)
